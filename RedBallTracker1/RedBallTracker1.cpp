...
int main() {
	...
	cv::Mat imgThresh;
	int imgCenterX = 320; // 480/2, (int)imgOriginal.rows / 2;  // move into while loop below - img doesn't exist yet
	int imgCenterY = 240; // 640/2, (int)imgOriginal.cols / 2;  // move into while loop below - img doesn't exist yet
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {		// until the Esc key is pressed or webcam connection is lost
		...
		cv::inRange(imgHSV, cv::Scalar(0, 155, 155), cv::Scalar(0, 100, 100), imgThreshLow);
		cv::inRange(imgHSV, cv::Scalar(165, 155, 155), cv::Scalar(200, 255, 255), imgThreshHigh);
		...
		for (int i = 0; i < v3fCircles.size(); i++) {		// for each circle . . .
			...
			//std::cout << v3fCircles[i][0] << " " << v3fCircles[i][1] << "\n";
			if (v3fCircles[i][0] > imgCenterX)
			{
				std::cout << "X: " << v3fCircles[i][0] << ">" << imgCenterX << "\tball is too far right\n";
			}
			else if (v3fCircles[i][0] < imgCenterX)
			{
				std::cout << "X: " << v3fCircles[i][0] << "<" << imgCenterX << "\tball is too far left\n";
			}
			if (v3fCircles[i][1] > imgCenterY)
			{
				std::cout << "Y: " << v3fCircles[i][1] << ">" << imgCenterY << "\tball is too far down\n";
			}
			else if (v3fCircles[i][1] < imgCenterY) 
			{
				std::cout << "Y: " << v3fCircles[i][1] << "<" << imgCenterY << "\tball is too far up\n";
			}
